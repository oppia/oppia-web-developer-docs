...
- Do not use `str()` under any circumstances. Please try to use `python_utils.convert_to_bytes()` or the `b'` prefix for the strings used in webapp2\'s built-in methods or for strings used directly in NDB datastore models. If you need to cast ints/floats to strings, please use `python_utils.UNICODE()` instead but please try to avoid casting strings. There is no need to prefix any other strings as all strings are prefixed with `u'` by default.

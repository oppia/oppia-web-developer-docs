...
Up till this point can all be done in one commit (or possibly several smaller ones on the same branch). Run ./scripts/test.sh, commit and then request a code review. This branch should be able to be merged in to develop after that code review if you so choose, since it should not impact the front end to record more unused events. If you want to keep working on this branch, make sure you periodically check to make sure you merge develop into your branch to keep up to date. After each merge, run tests to make sure that your changes do not cause any conflicts with the new code.

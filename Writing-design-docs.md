...
- Make sure that you understand the high-level goals of the project, then add increasingly more detail once that's established. Consider using the following strategy:
  1. Ensure there's an established product design from which the technical design is derived
...
- When breaking down a project, think of roughly each pull request needs to be created in order to fulfill your implementation plan. One task should correspond to a single PR, and your document should enumerate all of the tasks needed to complete the project.

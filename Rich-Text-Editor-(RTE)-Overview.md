...
In the decorator we obtain a list of component definitions from `rteHelperService.getRichTextComponents()`, and register each component as a tool with textAngular using `taRegisterTool`. In this call we can specify the tool name, the action to take on click, when the tool is disabled, and more.
## sanitizeHtmlForRte

...
* [Add dependencies](#add-dependencies)
  * [Requirements for new dependencies](#requirements-for-new-dependencies)
...
All new dependencies must be approved by @oppia/dependency-reviewers, who will check for the following criteria. Each new dependency must satisfy all of the conditions on one of the following lists:

**For Large Open-Source Dependencies:** Open-source software is often maintained by small groups of people and open to contributions from pretty much anyone. Therefore, we evaluate its safety by its security practices and history. Reviewers may use [OpenSSF Scorecards](https://scorecard.dev) to automate some of these checks.
...
* Code changes are secured by one of the following:
  * [ ] **Only a small group of people have commit access:** If only one or two people make commits, code review is less important (and less practical).
  * [ ] **Code review is enforced:** For larger groups of developers, code review is a valuable safeguard against both accidental and intentional security bugs.
* [ ] **No binary artifacts:** Binary artifacts are acceptable only if they are obviously not for use in production or obviously not supposed to contain code. By "obviously" we mean that any production code referencing them as code sources or executables would look clearly dangerous to a reviewer (e.g. image files or test assets).
* [ ] **Any unfixed vulnerabilities reported by GitHub are minor or not relevant:** You can use the OpenSSF Scorecard or check GitHub's [vulnerability database](https://github.com/advisories) directly.
* [ ] **Compatible license:** Must be compatible with Apache v2.0.
* [ ] **Adoption Scope:** How broadly has the dependency been adopted? The project should have one of the following (These particular thresholds are somewhat arbitrary--the goal is to ensure that the dependency is used broadly enough that malicious behavior would likely have been detected.):
     * At least 10 dependents where each dependent has at least 100 stars on GitHub.
     * At least 1000 stars on GitHub and at least 10,000 dependent repositories on GitHub.
...
**For Small, Un-Compiled Open-Source Dependencies:** For small open-source dependencies where the code is executed directly, we can review the source code manually.

* [ ] **We have manually reviewed the source code for the dependency and confirmed that it is safe:** This manual review must be done by a developer familiar with how to securely program in the language the dependency is written in.
* [ ] **Dependency compilation is safe.** This is essential because if the source code of a dependency is compiled and the compiled form is downloaded and executed, we cannot inspect the compiled form to ensure it is safe. We need to confirm one of the following:
  * [ ] **The dependency is not compiled at all.** For example, shell scripts.
  * [ ] **The dependency is compiled, and we have confirmed that the inspected source code produces the pinned compiled version bit-for-bit:** For example, locally compile the inspected source code and check that the output matches what we install.
* [ ] **Our reference to the dependency is pinned by a hash.** To ensure malicious code cannot be introduced after our review, we must pin a hash of the specific code we reviewed. This could be a commit hash, for example. Note that we must verify for ourselves that the hash matches the code we reviewed. A commit hash from GitHub satisfies this requirement. However, if we reviewed the code of a Python dependency on GitHub and then wanted to rely on the checksum generated by pip, we would need to verify that the code downloaded by pip exactly matches the code we reviewed on GitHub.
* [ ] **Compatible license:** Must be compatible with Apache v2.0.

**For Proprietary Dependencies:** Proprietary software is generally produced by companies in-house using procedures not available for our review and may be closed-source. We therefore have to evaluate their safety by the reputation of the company making it.

* [ ] **Compatible license:** Must be compatible with Apache v2.0.
* [ ] **Reputable author:** We're trusting the company to develop its software securely, so we must be confident that we can trust the author to (a) not be malicious, (b) carefully implement security best-practices to secure their software, and (c) keep their development systems safe from malicious actors. Since we cannot evaluate the software or how it's developed directly, this reputation bar is very high.
* [ ] **Actively maintained based on recent release history:** Note that we have to rely on releases instead of commits because proprietary software is often closed-source.
